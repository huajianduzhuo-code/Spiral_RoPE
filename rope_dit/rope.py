...
class VisionRotaryEmbedding(nn.Module):
    ...
    def forward(self, x): 
        # x shape: (B, N, num_heads, head_dim)
        assert x.ndim == 4, f"Input must have shape (B, N, num_heads, head_dim), but got {x.shape} with {x.ndim} dimensions"
        ...
        # Handle same_theta mode for rotated RoPE
        if self.same_theta and self.rotate > 0:
            assert freqs.shape[1] % 4 == 0
            # Reshape and repeat frequencies for each rotation direction
        ...
        # For 4D input: (B, N, num_heads, head_dim)
        # freqs should be (N, 1, head_dim) to broadcast across B and num_heads
